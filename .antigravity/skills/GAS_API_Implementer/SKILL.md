...
2.  **批次處理策略 (Batch Strategy)**
    ...
    * **規範**: 
        * `savePSI` 接收一個包含 7 個月份（1 Context + 6 Forecast）資料的扁平陣列。
        * 先在記憶體中將 Payload 中的資料依照 `Year` 分組 (Group by Year)。
...
**注意**: 此 API 必須支援 **全量讀取**。當不帶 `client` 與 `model_spec` 參數時，必須回傳資料庫中 **所有產品** 的 Context 與 Forecast 資料。
  ...
  "data": [
    {
      ...
      "prev_item":  {
        "date_month": "2025-12",  "forecast_p": 605, "forecast_s": 2, "forecast_i": 88, "actual_p": 0, "actual_s": 0, "actual_i": 0  },
      "forecast_items": [
        { "date_month": "2026-01", "forecast_p": 605, "forecast_s": 2, "forecast_i": 88, "actual_p": 0, "actual_s": 0, "actual_i": 0 },
        { "date_month": "2026-02", "forecast_p": 605, "forecast_s": 2, "forecast_i": 88, "actual_p": 0, "actual_s": 0, "actual_i": 0 },
        ... // 共 6 筆
      ]
    },
    ...
...
3.  **不可信任前端**: 即使前端說它是數字，後端運算前最好再做一次 `Number()` 或 `parseInt()` 轉換 (特別是 `forecast_p`, `forecast_s`)。
